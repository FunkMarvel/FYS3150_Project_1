...
\end{cpp}
For the specialized algorithm, the reciprocals of the new diagonal elements are precalculated as\footnote{Note that the indexing here is one off from the formula derived in section II. This is because arrays in C++ start indexing at 0, so in order to match the code the indexing in the formula has been altered.}
\begin{align*}
\frac{1}{\tilde{d}_{i-1}} = \frac{i}{i+1},
\end{align*}
